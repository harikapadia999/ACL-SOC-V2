...
//           toast.error(
//             "Network or CORS Error - If this persists, please login again"
//           );
//           if (!ENV.IS_DEV) {
    ...
    this.client.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        if (error.code === "ERR_NETWORK") {
          ...
          // Suppress loud 500 console errors to avoid triggering the platform error interceptor
          console.warn("Backend 500 Error suppressed for:", error.config?.url);
        } else if (
          error.response?.status === 401 ||
          error.response?.status === 403
        ) {
