...
//         apiClient
//           .get<any>("/api/v1/dashboard/metrics", {
//             params: { tenant_id: tenantId },
//           })
//           .catch((e) => {
//             if (e.response?.status === 401 || e.response?.status === 403)
//               throw e;
//             return null; // metrics are optional, transformBackendMetrics handles fallback
//           }),
//         apiClient
...
//             if (e.response?.status === 401 || e.response?.status === 403)
//               throw e;
//             return { data: [] }; // Fallback to empty array if backend is failing so we don't crash
//           }),
//       ]);
...
//       const metrics = transformBackendMetrics(
...
//     const response = await apiClient.get("/threat-intel/providers");
//     return response.data;
//   },
      ...
      const [metricsResponse, alertsResponse] = await Promise.all([
        ...
        apiClient
          ...
          .catch((e) => {
            if (e.response?.status === 401 || e.response?.status === 403)
              throw e;
            console.warn("Alerts backend failure (e.g. 500):", e.message);
            return { data: [] }; // Fallback to empty array if backend is failing so we don't crash
          }),
      ]);
  ...
  getAllAlerts: async (): Promise<Alert[]> => {
    try {
      ...
      const response = await apiClient
        ...
        .catch((e) => {
          if (e.response?.status === 401 || e.response?.status === 403) throw e;
          console.warn("Alerts list backend failure:", e.message);
          return { data: [] };
        });
      if (!response || !Array.isArray(response.data)) {
        return [];
